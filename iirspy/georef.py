"""
Co-register an IIRS strip to LOLA topography and hand back GCPs + a geotransform and a GLT.

The supplied geometry csv is off by up to 10 km, particularly at the poles. Use image feature
matching to produce ground control points in camera (x,y). These are converted via a TPS warp to
a GLT so the strip can be projected into north/south polar stereographic or equatorial cylindrical.
...
def warp(cube, reg, cfg, resampling=Resampling.bilinear, window=None):
    ...
    the full grid is tens of GB. For a scene-sized warp, use a GLT (:func:`make_glt`) instead --
    this is for small targeted AOIs (e.g. a quick look in QGIS), never a whole scene.
    """
...
# keeps one unblended L1/L2 spectrum. Built by warping two index bands through `project` at
# `Resampling.nearest`, so `apply_glt(cube, glt)` reproduces `project(cube, ..., nearest)` bit for
# bit. A nearest sample reads one source pixel, so each band keeps its own nodata with no
# indicator planes to unify.
#
...
def _glt_parser():
    ...
    ap = argparse.ArgumentParser(description="Build a scene's GLT from its merged GCPs.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    args = _glt_parser().parse_args(argv)
    fgcps = Path(args.gcps) if args.gcps else solve.merged_gcps_path(args.sid, args.group)
